...
void UEdGraphSchema_FroxEditor::GetContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	const FName DefaultSectionName = FName(TEXT("Default"));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().Delete));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().Cut));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().Copy));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().Paste));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().SelectAll));
	Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGenericCommands::Get().Duplicate));

	if (Context->Pin)
	{
		Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGraphEditorCommands::Get().BreakPinLinks));
	}
	else
	{
		Menu->AddMenuEntry(DefaultSectionName, FToolMenuEntry::InitMenuEntry(FGraphEditorCommands::Get().BreakNodeLinks));
	}
	
	Super::GetContextMenuActions(Menu, Context);
}

void UEdGraphSchema_FroxEditor::TrySetDefaultValue(UEdGraphPin& InPin, const FString& NewDefaultValue, bool bMarkAsModified) const
{
	Super::TrySetDefaultValue(InPin, NewDefaultValue, bMarkAsModified);
}

void UEdGraphSchema_FroxEditor::TrySetDefaultObject(UEdGraphPin& InPin, UObject* NewDefaultObject, bool bMarkAsModified) const
{
	Super::TrySetDefaultObject(InPin, NewDefaultObject, bMarkAsModified);
}

void UEdGraphSchema_FroxEditor::TrySetDefaultText(UEdGraphPin& InPin, const FText& InNewDefaultText, bool bMarkAsModified) const
{
	Super::TrySetDefaultText(InPin, InNewDefaultText, bMarkAsModified);
}
